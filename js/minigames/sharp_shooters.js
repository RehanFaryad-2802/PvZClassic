...
const SharpShooters = (() => {
  ...
  const DIFFICULTIES = [
    { name:"Easy",   pips:1, speedMult:1.0,  spawnMult:1.0,  bossChance:0,    batChance:0,    reward:{seeds:1,  coins:6}  },
    { name:"Medium", pips:2, speedMult:1.35, spawnMult:0.72, bossChance:0.08, batChance:0.12, reward:{seeds:2,  coins:14} },
    { name:"Hard",   pips:3, speedMult:1.75, spawnMult:0.52, bossChance:0.15, batChance:0.20, reward:{seeds:4,  coins:26} },
    { name:"Expert", pips:4, speedMult:2.2,  spawnMult:0.38, bossChance:0.22, batChance:0.28, reward:{seeds:7,  coins:42} },
    { name:"Insane", pips:5, speedMult:2.8,  spawnMult:0.27, bossChance:0.30, batChance:0.35, reward:{seeds:11, coins:65} },
  ];
  ...
  function startGame(diffIdx = 0) {
    ...
    state        = { score:0, combo:1, escapes:0, timeLeft:CONFIG.GAME_DURATION, running:false };
    activeDemons = []; frenzyActive = false; freezeActive = false;
    ...
    gameTimer = setInterval(() => {
      ...
      if (fEl) fEl.style.width = (state.timeLeft/CONFIG.GAME_DURATION)*100+"%";
      if (state.timeLeft <= 0) {
    ...
    const spawnMs = Math.max(CONFIG.SPAWN_INTERVAL_MIN,
      Math.round(CONFIG.SPAWN_INTERVAL_BASE * DIFFICULTIES[currentDiff].spawnMult));
    spawnTimer = setInterval(() => { if (state.running) spawnDemon(DIFFICULTIES[currentDiff]); }, spawnMs);

    lastTs = null; cancelAnimationFrame(rafId);
  ...
  function tick(ts) {
    ...
    for (let i = activeDemons.length-1; i >= 0; i--) {
      ...
      if (d.x + CONFIG.DEMON_SIZE < 0) {
        d.el.remove(); activeDemons.splice(i, 1);
        state.escapes++; state.combo = 1; updateHUD();
  ...
  function spawnDemon(diff) {
    ...
    let type;
    const r = Math.random();
    if      (r < CONFIG.POWERUP_CHANCE)                                type = "pu_" + Object.keys(CONFIG.POWERUP_TYPES)[Math.floor(Math.random()*3)];
    else if (r < CONFIG.POWERUP_CHANCE + diff.bossChance)              type = "brute";
    else if (r < CONFIG.POWERUP_CHANCE + diff.bossChance + diff.batChance) type = "bat";
    else { const base=["imp","armored","ice"]; type=base[Math.floor(Math.random()*base.length)]; }
    ...
    const speedPx = isPU ? 70 : Math.min(CONFIG.MARCH_SPEED_MAX, CONFIG.MARCH_SPEED_BASE * diff.speedMult * cfg.speed);
    ...
    el.addEventListener("click", (e) => { e.stopPropagation(); shootDemon(demon); });
    el.addEventListener("touchstart", (e) => { e.preventDefault(); e.stopPropagation(); shootDemon(demon); }, { passive:false });
  }
  ...
  function onShoot(e) {
    ...
    activeDemons.forEach(d => {
      ...
      const dist = Math.sqrt(Math.pow((dr.left+dr.width/2)-arenaRect.left-cx,2)+Math.pow((dr.top+dr.height/2)-arenaRect.top-cy,2));
  ...
  function shootDemon(demon) {
    ...
    if (isPU) { activatePowerup(puKey, demon); removeDemon(demon); return; }
  ...
  function activatePowerup(key, demon) {
    ...
    if (key === "freeze") {
      ...
      clearTimeout(freezeTimeout); freezeActive=true;
      arena.classList.add("ss-frozen");
      freezeTimeout = setTimeout(() => {
        activeDemons.forEach(d => d.frozen=false);
        freezeActive=false; arena.classList.remove("ss-frozen"); showPowerupLabel("");
      }, CONFIG.FREEZE_DURATION);
    } else if (key === "frenzy") {
      frenzyActive=true; arena.classList.add("ss-frenzy");
      clearTimeout(frenzyTimeout);
      frenzyTimeout = setTimeout(() => { frenzyActive=false; arena.classList.remove("ss-frenzy"); showPowerupLabel(""); }, CONFIG.FRENZY_DURATION);
    } else if (key === "nuke") {
      ...
      toKill.forEach(d => {
        if (!d.type.startsWith("pu_")) {
          ...
          d.dead=true; d.el.classList.add("ss-die");
          setTimeout(() => d.el.remove(), 350);
        }
      });
      activeDemons=[]; updateHUD();
    }
  }
  ...
  function endGame() {
    ...
    activeDemons.forEach(d => d.el.remove()); activeDemons=[];
